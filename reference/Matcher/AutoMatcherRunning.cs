...
namespace PCM_Inspection_Demo.Matcher
{
    public class AutoMatcherRunning
    {
        ...
        private IMatcher Matcher { get; set; } = null;

        private readonly string _tempFolder = Path.Combine(AppDomain.CurrentDomain.BaseDirectory, "auto-matcher");

        public AutoMatcherRunning(string refPath, List<string> testPaths, IMatcher matcher)
        {
            RefPath = refPath;
                ...
                foreach (var testPath in TestPaths)
                {
                    ...
                    try
                    {
                        ...
                        using (var test = new Bitmap(testPath))
                        {
                            ...
                            var summary = IMatcher.FormatTransformForLog(result);
        ...
        private void SaveAlignedDebugImage(Bitmap testImage, string originalPath, IMatcher.MatchResult result)
        {
