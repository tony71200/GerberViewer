...
| 5 Load/display TIFF | Partial | `CreateGerberSampleControl.cs` | Warning: `msbuild` unavailable | Static flow review | Tab 2 now decodes selected raster via `ImageRead.ReadBitmap` and assigns a cloned bitmap to `sampleWindow`; HALCON-specific TIFF API not added in this pass. |
...
| 7 Draw red grid/order | Done | `CreateGerberSampleControl.cs` | Warning: `msbuild` unavailable | Static flow review | Image load/config change calculates layout and displays red rectangles plus order labels. |
