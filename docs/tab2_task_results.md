...
| 7 Draw red grid/order | Done | `CreateGerberSampleControl.cs`, `GerberSampleWindow.cs` | Warning: `msbuild` unavailable | Static flow review | Image load/config/Refresh change calculates layout and displays red margin rectangles plus visible yellow boxed order labels without modifying EWindowControl project code. |
