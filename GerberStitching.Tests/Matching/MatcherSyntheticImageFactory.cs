...
namespace GerberStitching.Tests.Matching
{
    public static class MatcherSyntheticImageFactory
    {
        public static Bitmap CreateAsymmetricBgrBitmap(int width, int height)
        {
            ...
            using (var g = Graphics.FromImage(bitmap))
            {
                ...
                using (var yellow = new SolidBrush(Color.FromArgb(230, 220, 30)))
                {
                    ...
                    g.FillPolygon(blue, new[] { new Point(width - width / 5, height - height / 6), new Point(width / 2, height - height / 4), new Point(width - width / 3, height / 2) });
        ...
        public static Mat CreateAsymmetricMono8Mat(int width, int height)
        {
            ...
            Cv2.Circle(mat, new Point(width * 3 / 5, height / 3), width / 10, Scalar.All(180), -1);
            Cv2.Line(mat, new Point(width / 4, height * 3 / 4), new Point(width - width / 6, height / 2), Scalar.All(240), 3);
            mat.Set<byte>(1, 1, 37);
        ...
        public static Mat CreateAsymmetricMono16Mat(int width, int height)
        {
            ...
            Cv2.Circle(mat, new Point(width * 3 / 5, height / 3), width / 10, Scalar.All(32768), -1);
            Cv2.Line(mat, new Point(width / 4, height * 3 / 4), new Point(width - width / 6, height / 2), Scalar.All(60000), 3);
