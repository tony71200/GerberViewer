...
namespace GerberViewer.Stitching.Imaging
{
    ...
    public sealed class PreparedSampleRun : IDisposable
    {
        ...
        {
            if (sourceImage == null || !sourceImage.IsInitialized()) throw new ArgumentException("Owned source image is required.", nameof(sourceImage));
            if (processedImage == null || !processedImage.IsInitialized()) throw new ArgumentException("Owned processed image is required.", nameof(processedImage));
            if (sourceWidth <= 0 || sourceHeight <= 0 || processedWidth <= 0 || processedHeight <= 0) throw new ArgumentOutOfRangeException("Image dimensions must be positive.");
            if (configSnapshot == null) throw new ArgumentNullException(nameof(configSnapshot));
            if (layout == null || layout.Tiles == null) throw new ArgumentNullException(nameof(layout));
            SourceImage = sourceImage; ProcessedImage = processedImage; SourceWidth = sourceWidth; SourceHeight = sourceHeight; ProcessedWidth = processedWidth; ProcessedHeight = processedHeight;
            ConfigSnapshot = configSnapshot; Layout = layout; TilesByOrder = layout.Tiles.OrderBy(t => t.OrderIndex).ToList().AsReadOnly(); PreprocessMetadata = metadata ?? new ImagePreprocessMetadata();
        }
    ...
    public interface ISamplePreparationService { PreparedSampleRun Prepare(HObject sourceImage, GerberSampleConfig config, CancellationToken cancellationToken); }
    ...
    public sealed class SamplePreparationService : ISamplePreparationService
    {
        ...
        {
            if (sourceImage == null || !sourceImage.IsInitialized()) throw new ArgumentException("Source image is required.", nameof(sourceImage));
            ...
            {
                if (ownedSource != null && ownedSource.IsInitialized()) ownedSource.Dispose();
                if (processed != null && processed.IsInitialized()) processed.Dispose();
                throw;
            }
        }
        ...
        private static HObject Preprocess(HObject source, Size sourceSize, GerberSampleConfig config, CancellationToken ct)
        {
            ...
            if (config.PreprocessMode == SamplePreprocessMode.None || (width == sourceSize.Width && height == sourceSize.Height)) HOperatorSet.CopyImage(source, out result);
            else HOperatorSet.ZoomImageSize(source, out result, width, height, "constant");
            if (config.InvertImage)
            {
                HObject inverted = null;
                try { HOperatorSet.InvertImage(result, out inverted); result.Dispose(); result = inverted; inverted = null; }
                finally { if (inverted != null && inverted.IsInitialized()) inverted.Dispose(); }
            }
            return result;
        }
        ...
        private static Size GetSize(HObject image)
        {
            ...
            try { HOperatorSet.GetImageSize(image, out w, out h); return new Size(w.I, h.I); }
            finally { if (w != null) w.Dispose(); if (h != null) h.Dispose(); }
        }

        private static GerberSampleConfig CloneConfig(GerberSampleConfig c)
        {
            return new GerberSampleConfig { SourceRasterPath = c.SourceRasterPath, OutputDirectory = c.OutputDirectory, Rows = c.Rows, Columns = c.Columns, CropOrder = c.CropOrder, StartOrder = c.StartOrder, InvertImage = c.InvertImage, OverlapValue = c.OverlapValue, OverlapUnit = c.OverlapUnit, PreprocessMode = c.PreprocessMode, KeepAspectRatio = c.KeepAspectRatio, OutputFormat = c.OutputFormat, TileNamePattern = c.TileNamePattern, ProcessedWidth = c.ProcessedWidth, ProcessedHeight = c.ProcessedHeight, PadColor = c.PadColor };
        }
    }
}
