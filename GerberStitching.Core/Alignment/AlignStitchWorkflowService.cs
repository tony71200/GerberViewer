...
namespace GerberViewer.Stitching.Alignment
{
    ...
    public sealed class AlignStitchWorkflowService
    {
        ...
        private static SampleTileInfo FindTile(SampleManifest m, CapturedImageInfo c) { return m.Tiles.FirstOrDefault(t => t.Row == c.Row && t.Column == c.Column) ?? new SampleTileInfo { Row = c.Row, Column = c.Column, ExpectedX = c.Column * 1.0, ExpectedY = c.Row * 1.0, ExpectedPath = c.FilePath, Width = c.Width, Height = c.Height, OrderIndex = c.OrderIndex }; }
