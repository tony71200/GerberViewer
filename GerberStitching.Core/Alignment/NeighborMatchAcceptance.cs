...
namespace GerberViewer.Stitching.Alignment
{
    ...
    public static class NeighborMatchAcceptance
    {
        public static NeighborAcceptanceResult Validate(
            MatchResult coarse, 
            MatchResult refined, 
            Transform2D targetToAnchor, 
            double expectedTranslationX, 
            ...
            double overlapRatio)
        {
            ...
            if (result == null || !result.Success) 
                return Reject("No successful neighbor matcher result.");
            if (targetToAnchor == null) 
                return Reject("TargetToAnchorTransform is missing.");
            ...
            if (coarse == null || !coarse.Success || coarse.RawScore < options.PhaseMinResponse) 
                return Reject("Phase correlation score below threshold.");
            if (refined != null && refined.Success && refined.RawScore < options.MinCorrelation) 
                return Reject("ECC correlation below threshold.");
            var rotationDeg = Math.Atan2(matrix[1, 0], matrix[0, 0]) * 180.0 / Math.PI;
            if (Math.Abs(rotationDeg) > options.MaxAbsRotationDeg) 
                return Reject("Pair rotation exceeds MaxAbsRotationDeg.");
            ...
            if (Math.Sqrt(dx * dx + dy * dy) > options.MaxTranslationPixels) 
                return Reject("Pair translation deviation exceeds MaxTranslationPixels.");
